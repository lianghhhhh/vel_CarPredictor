...
def getInputData(data_path):
    ...
    with open(data_path, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            vels.append([float(row[1]), float(row[2]), float(row[3]), float(row[4])])  # target & current velocities
            states.append([float(row[5]), float(row[6]), float(row[7])])  # car states(x, z, angle)

    vels = vels[:-1]
    state_delta = []
    for i in range(len(states) - 1):
        global_dx = states[i + 1][0] - states[i][0]
        global_dz = states[i + 1][1] - states[i][1]
        d_angle = states[i + 1][2] - states[i][2]
        if d_angle > 180:
            d_angle -= 360
        elif d_angle < -180:
            d_angle += 360
