...
def getInputData(data_path):
    ...
    for i in range(len(df)):
        ...
        for k in range(20):  # 20 target points
        ...
        for k in range(10):  # 10 obstacles according to target points
            obstacle_flag = df[f'obstacle_flag_{k}'].values[i]
            if obstacle_flag == 1.0: # have obstacle
                ...
                local_x = delta_x * np.cos(-curr_angle) - delta_y * np.sin(-curr_angle)
                local_y = delta_x * np.sin(-curr_angle) + delta_y * np.cos(-curr_angle)
                input_vector.append(obstacle_flag)
                input_vector.append(local_x)
                input_vector.append(local_y)
            else: # no obstacle, use flag and large distance to indicate
                input_vector.append(obstacle_flag)
                input_vector.append(0.0) # use 0.0 for no obstacles
                input_vector.append(0.0)

        for k in range(20):  # 20 planned path points
            ...
            # calculate offset to target point instead of current position
            # focus on path planning rather than localization error
            offset_x = planned_x - target_x
            offset_y = planned_y - target_y
            offset_x_local = offset_x * np.cos(-curr_angle) - offset_y * np.sin(-curr_angle)
            offset_y_local = offset_x * np.sin(-curr_angle) + offset_y * np.cos(-curr_angle)
            offset_angle = planned_angle - target_angle
            offset_angle = (offset_angle + np.pi) % (2 * np.pi) - np.pi  # normalize to [-pi, pi]
            output_vector.append(offset_x_local)
            output_vector.append(offset_y_local)
            output_vector.append(offset_angle)

        inputs.append(input_vector)
        outputs.append(output_vector)

    # scale up outputs, since the offset is usually small and we want to emphasize it during training
    outputs = np.array(outputs) * 10.0
